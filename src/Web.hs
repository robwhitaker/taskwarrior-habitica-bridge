...
import Network.HTTP.Req (Req, ReqBodyJson(..), Url, Option, Scheme(Https), (/:), (=:))
import qualified Data.UUID as UUID
import Data.Text (Text)
import qualified Data.Text as T
import Data.Aeson (parseJSON)
import Data.Aeson.Types (FromJSON, parseEither, Value(Null))
...
habiticaApiV3 =
    Req.https "habitica.com" /: "api" /: "v3"

tasksEndpoint :: Url 'Https
tasksEndpoint =
    habiticaApiV3 /: "tasks" /: "user"

taskEndpoint :: UUID -> Url 'Https
taskEndpoint (UUID taskId) =
    habiticaApiV3 /: "tasks" /: UUID.toText taskId
...
habiticaHeaders textUUID textApiKey =
    ...
    <$> UUID.fromText textUUID <*> UUID.fromText textApiKey

{- RUNNING REQUESTS -}

runHabiticaReq :: FromJSON a => Req (Either String (HabiticaResponse a)) -> IO (HabiticaResponse a)
...
runHabiticaReq' req = do
    catch (Req.runReq reqConf req) (\(_ :: Req.HttpException) -> return Null) >> return ()
  where
    -- Throw errors out the window for now
    -- TODO: Actually handle them properly
    reqConf = Req.defaultHttpConfig {
...
habiticaCreateOrUpdateRequest :: HabiticaHeaders -> HabiticaTask -> Req (Either String (HabiticaResponse HabiticaTask))
habiticaCreateOrUpdateRequest (HabiticaHeaders headers) habiticaTask@(HabiticaTask task) =
    let
        mkReq = case taskHabiticaId task of
            Just id ->
                Req.req Req.PUT (taskEndpoint id)

            Nothing ->
                Req.req Req.POST tasksEndpoint
    in
        mkReq (ReqBodyJson habiticaTask) Req.jsonResponse headers
            >>= return . parseEither parseJSON . Req.responseBody

habiticaGetTasks :: HabiticaHeaders -> Maybe Text -> Req (Either String (HabiticaResponse [HabiticaTask]))
habiticaGetTasks (HabiticaHeaders headers) maybeQueryParam =
    Req.req Req.GET tasksEndpoint Req.NoReqBody Req.jsonResponse (headers <> Req.queryParam "type" maybeQueryParam)
        >>= return . parseEither parseJSON . Req.responseBody

habiticaGetTask :: HabiticaHeaders -> UUID -> Req (Either String (HabiticaResponse HabiticaTask))
